...
<nav class="navbar navbar fixed-top navbar-expand-lg navbar-transparent bg-light " class="navbar sticky-top" >
 ...
 <h2 style="color: #CCCCB2">ACCOUNT UPDATE FORM</h2>       
		...
		<?php
				...
				if(isset($_POST['up_id'])){		
					$up_id = $_POST['up_id'];			
			 		...
			 		$upQuery = "UPDATE tbluser SET password ='$password' WHERE id = {$up_id}";
				...
				if ($result->num_rows > 0) {
				    ...
				    while($row = $result->fetch_assoc()) {
						        ...
						        <button  type=\"text\" type=\"submit\" name=\"password\" class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#exampleModalCenter\">
								  Update
								</button>
						        		<input type= \"hidden\" name = \"up_id\" value =\" {$row['id']}\">
						        		<div class=\"form-group\">
									    
									    <textarea type=\"text\" type=\"submit\" name=\"password\"class=\"form-control\" id=\"exampleFormControlTextarea1\" rows=\"3\" placeholder=\"Write and change password here....\" autofocus></textarea>
									  </div>
						        </form>
						        	"."</td>
						        	";
